...
function DealCard({ deal, onSave, client, onLocalUpdate }: { deal: Deal; onSave: (u: Partial<Deal> & { id: string }) => Promise<void>; client: Client; onLocalUpdate?: (u: Partial<Deal> & { id: string }) => void }) {
  ...
  // Modal para confirmar cambio de modalidad de pago (contado ⇄ financiado)
  const [confirmChangeModalidad, setConfirmChangeModalidad] = useState(false);
    ...
    <div className="rounded-2xl border transition-all overflow-hidden" style={{ borderColor: `color-mix(in srgb, ${pc.color} 30%, transparent)`, background: exp ? "var(--surface-light)" : "var(--surface)" }}>
      ...
      {exp && (
        <div className="px-5 pb-5 animate-scale-in">
                              ...
                              <div className="space-y-2 p-3 rounded-xl bg-[var(--surface-light)] border border-red-500/20" onClick={e => e.stopPropagation()}>
                                ...
                                <div className="flex gap-2">
                                  ...
                                  <button onClick={async () => {
                                    if (!noCondMotivo.trim() || !noCondImg) return;
                                    setSaving(true);
                                  ...
                                  }} disabled={!noCondMotivo.trim() || !noCondImg || saving} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-[11px] font-semibold bg-[var(--red)] text-white disabled:opacity-50">{saving ? <Loader2 size={11} className="animate-spin" /> : <XCircle size={11} />}Confirmar</button>
                                </div>
                              </div>
                            )}
          ...
          {/* Modal de confirmación para cambiar modalidad de pago */}
          {confirmChangeModalidad && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => setConfirmChangeModalidad(false)}>
              <div className="bg-[var(--surface-light)] rounded-2xl border border-[var(--border)] p-5 max-w-md w-full" onClick={e => e.stopPropagation()}>
                <div className="flex items-center gap-2 mb-3">
                  <AlertTriangle size={20} className="text-[var(--yellow)]" />
                  <h3 className="text-[16px] font-semibold">Cambiar modalidad de pago</h3>
                </div>
                <p className="text-[13px] text-[var(--text-secondary)] mb-4">¿Seguro que quieres cambiar la modalidad? Se conservan los datos del negocio pero la modalidad actual ({deal.modalidad_pago === "contado" ? "Contado" : "Financiado"}) se resetea y podrás elegir de nuevo.</p>
                <div className="flex gap-3">
                  <button onClick={() => setConfirmChangeModalidad(false)} className="flex-1 px-4 py-2.5 rounded-xl text-[13px] font-medium border border-[var(--border)] text-[var(--text-muted)] hover:text-[var(--text-primary)]">Cancelar</button>
                  <button
                    onClick={async () => {
                      setSaving(true);
                      // Resetear modalidad_pago y los campos asociados; se conservan cuotas_total/fecha_corte por si el usuario los quiere reutilizar
                      const updates: Partial<Deal> = { modalidad_pago: "" };
                      if (deal.modalidad_pago === "contado") {
                        // Limpiar datos de contado
                        updates.fecha_limite_pago = "";
                      }
                      await sv(updates);
                      setConfirmChangeModalidad(false);
                      setSaving(false);
                    }}
                    disabled={saving}
                    className="flex-1 px-4 py-2.5 rounded-xl text-[13px] font-semibold bg-[var(--accent)] text-white disabled:opacity-50"
                  >
                    {saving ? <Loader2 size={13} className="animate-spin inline" /> : "Sí, cambiar"}
                  </button>
                </div>
              </div>
            </div>
